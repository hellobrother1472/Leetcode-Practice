...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        queue<pair<int,pair<int,int>>> pq; // Price , <stop,node>
        ...
        while(!pq.empty()){
            ...
            for(auto it : adj[node]){
                ...
                if( price + addPrice < totalPrice[newNode] && stop <= k){
                    totalPrice[newNode] = price + addPrice;
                    pq.push({price + addPrice,{stop+1,newNode}});
                }
